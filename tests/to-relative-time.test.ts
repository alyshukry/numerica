...
])("toRelativeTime() — basic past times", (date, options, expected) => {
    it(`should return "${expected}"`, () => {
        expect(toRelativeTime(date, options)).toBe(expected)
    })
})
...
])("toRelativeTime() — future times", (date, options, expected) => {
    it(`should return "${expected}"`, () => {
        expect(toRelativeTime(date, options)).toBe(expected)
    })
})
...
])("toRelativeTime() — lod option", (date, options, expected) => {
    it(`should return "${expected}"`, () => {
        expect(toRelativeTime(date, options)).toBe(expected)
    })
})
...
])("toRelativeTime() — custom separator", (date, options, expected) => {
    it(`should return "${expected}"`, () => {
        expect(toRelativeTime(date, options)).toBe(expected)
    })
})
...
// Abbreviation mode
describe.each([
    [new Date(Date.now() - (HOUR + MINUTE)), { lod: 2, abbreviate: true }, "1h, 1min ago"],
    [new Date(Date.now() + (DAY + HOUR)), { lod: 2, abbreviate: true }, "in 1d, 1h"]
])("toRelativeTime() — abbreviate option", (date, options, expected) => {
    it(`should return "${expected}"`, () => {
        expect(toRelativeTime(date, options)).toBe(expected)
    })
})
...
describe.each([
    ...
    [new Date(Date.now() + HOUR), { includeSuffix: false }, "1 hour"]
])("toRelativeTime() — includeSuffix=false", (date, options, expected) => {
...
])("toRelativeTime() — edge and combined cases", (date, options, expected) => {
