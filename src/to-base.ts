 ...
 * 
 * @param n - The number to convert (as a number or string). Supports fractional values like `10.5` or `'A.8'`.
 * @param to - The target base to convert to (2–62).
 * @param from - The source base of the input number (default: 10).
 * @param precision - The maximum number of digits to include after the decimal point when converting fractional values (default: 8).
 * 
 ...
 * toBase('A.8', 10, 16)     // "10.5" (hexadecimal with fraction to decimal)
 * toBase('-2A.4', 10, 16)   // "-42.25" (negative fractional conversion)
 */

export function toBase(n: number | string, to: number, from: number = 10, precision = 8): string {
    ...
    const [intPartStr, fracPartStr = ''] = str.replace(/^-/, '').split('.') //splits number to integer part and decimal part and removes the negative sign
    ...
    const result = intResult.join('') + (fracResult.length ? '.' + fracResult.join('') : '')
